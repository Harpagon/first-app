		...
		}else
		{
			...
			if (find_vars !== undefined && find_vars.length > 0)
			{
				...
				if (know_vars !== undefined && know_vars.length > 0)
				{
					...
					var title = "Решение найдено";
					if (solvetion.length == 1)
						title = "Решение не найдено"
					res.render('solve',{title: title, lastSeparator:false,parent_razd:razdels.getPathById(razd),findable_vars:findable_vars,find_vars:ffvars,kv_to_add:kv_to_add,url:req.url, know_vars: know_vars, solvetion: solvetion});
				}else
				{
