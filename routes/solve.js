	...
	if(parent != "NOT_FOUND")
	{
		...
		}else
		{
			...
			if (find_vars !== undefined && find_vars.length > 0)
			{
				...
				{
					res.render('solve',{title:"Выбор известных переменных", lastSeparator:false,parent_razd:razdels.getPathById(razd),findable_vars:findable_vars,find_vars:ffvars,kv_to_add:kv_to_add});
				}
			}else
			{
